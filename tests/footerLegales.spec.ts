...
test.describe("Footer - Legales", () => {
    ...
    for (const linkData of LEGAL_LINKS) {
        ...
        test.describe(`Sección: ${linkData.sectionName}`, () => {
            ...
            test(`Debe mostrar la opción de Términos y condiciones con los atributos correctos`, { tag: ["@smoke", "@homePage", "@footer"] }, async ({ homePage }) => {
                const locator = linkData.getLocator(homePage);

                await test.step("Validar atributos del enlace en la interfaz", async () => {
                    await expect(locator).toBeVisible();
                    await expect(locator).toHaveAttribute("href", new RegExp(linkData.expectedPdf));
                    await expect(locator).toHaveAttribute("target", "_blank");
                });
            });

            test(`Debe abrir el PDF en una nueva pestaña al hacer click`, { tag: ["@smoke", "@homePage", "@footer"] }, async ({ homePage }) => {
                const locator = linkData.getLocator(homePage);

                const { documentPage, documentResponse } = await test.step("Ejecutar apertura e interceptar la respuesta de red", async () => {
                ...
                await test.step("Validar la integridad y meta-datos del archivo descargado", async () => {
                    expect.soft(documentResponse.status(), "El status HTTP no es 200").toBe(200);
                    expect.soft(documentResponse.headers()['content-type'], "El Content-Type no es de un PDF").toContain("application/pdf");
                    expect.soft(documentResponse.url(), "La URL del documento es incorrecta").toContain(linkData.expectedPdf);
                });
